                        ...
                        try {
                            ...
                            if (userdata.getStatus().equals("true")) {
                                try {
                                    ...
                                    boolean fileStatus = storageManager.moveFilesToSentPath(FileUploadService.this, mdata.message_type, filepath, userdata.getResult().getImage());
                                    if (fileStatus) {
                ...
                public void onResponse(Call<GroupImageModel> call, Response<GroupImageModel> response) {
                    ...
                    if (data != null && data.getSTATUS().equalsIgnoreCase(TRUE)) {
                        try {
                            ...
                            boolean fileStatus = storageManager.moveFilesToSentPath(FileUploadService.this, gdata.messageType, filepath, data.getRESULT().getUserImage());
                            if (fileStatus) {
                                JSONObject message = new JSONObject();
                    ...
                    try {
                        ...
                        if (userdata.get(Constants.TAG_STATUS).equals("true")) {
                            try {
                                ...
                                boolean fileStatus = storageManager.moveFilesToSentPath(FileUploadService.this, chdata.messageType, filepath, userdata.get(Constants.TAG_USER_IMAGE));
                                if (fileStatus) {
                                    JSONObject message = new JSONObject();
